...
def test(request):
    if request.method == "POST" and request.FILES['myfile']:
        ...
        img = scipy.ndimage.imread(myfile, mode='RGB')
        out = enhancer.process(img)
        rname = str(binascii.b2a_hex(os.urandom(15)))
        out.save('media/'+ rname +'.png')
        with open('media/'+ rname +'.png', 'rb') as fh:
            response = HttpResponse(fh.read(), content_type="application/vnd.ms-excel")
            response['Content-Disposition'] = 'inline; filename=' + 'media/'+ rname +'.png'
            return response
